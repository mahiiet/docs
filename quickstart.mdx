...
- **A Rise account** with a RiseID (get one at [app.rise.works](https://app.rise.works))
...
class RiseAuth {
  ...
  async authenticate(walletAddress, riseId) {
    try {
      ...
      // Step 2: Sign the message (in a real app, this would be done by the user's wallet)
  ...
  async getUserOrganizations() {
    try {
      ...
      const response = await axios.get(
        `${this.baseUrl}/v2/user/organizations`,
        { headers: this.headers }
      );
      
      console.log('✅ Organizations:', response.data.data.companies);
...
async function main() {
  try {
    ...
    await api.getUserOrganizations();
    
    // Get user teams
    await api.getUserTeams();
    
    // Get balance (replace with actual nanoid)
    // await api.getEntityBalance('te_your_team_nanoid_here');
